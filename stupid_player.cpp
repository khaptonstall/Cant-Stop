...
int const stupid_player::SELECT_DELAY = 2000;

stupid_player::stupid_player() {
	timer = 0;
	last_ticks = 0;
}

pair<int, int> stupid_player::select_dice(GameState* game_state, vector<pair<int, int> > rolled_pairs, Player* p, int selected_dice) {
	// Decision delay
	if (last_ticks == 0) {
...
int stupid_player::select_decision(GameState* game_state, int selected_decision) {
	if (last_ticks == 0) {
		last_ticks = SDL_GetTicks();
		return 0;
	}
	...
	if (probability > progress * (expected_progress * probability)) {
		return 1;
	}
	else {
		// cout << "Stopped" << endl;
